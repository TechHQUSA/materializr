...
| CONVERTED | src/app/Application_Dialogs.cpp:1617 | `if (materializr::amountLengthField("thrPitchAmt", materializr::tr("Pitch"), &m_threadPitch, /*allowSign=*/fals` |
| CONVERTED | src/app/Application_Dialogs.cpp:1625 | `if (materializr::parseLength(m_threadPitchBuf, v) && v >= 0.1f)` |
| CONVERTED | src/app/Application_Dialogs.cpp:1632 | `if (materializr::amountLengthField("thrDepthAmt", materializr::tr("Depth"), &m_threadDepth, /*allowSign=*/fals` |
| CONVERTED | src/app/Application_Dialogs.cpp:1640 | `if (materializr::parseLength(m_threadDepthBuf, v) && v >= 0.05f)` |
| CONVERTED | src/app/Application_Dialogs.cpp:1677 | `materializr::lengthField("##thrClr", &m_threadClearance);` |
| CONVERTED | src/app/Application_Dialogs.cpp:1691 | `materializr::lengthField("##thrGWidth", &m_threadGrooveWidth);` |
| percent | src/app/Application_Dialogs.cpp:2073 | `if (ImGui::SliderFloat(materializr::tr("Opacity"), &opacity, 0.05f, 1.0f, "%.2f")) {` |
| CONVERTED | src/app/Application_Dialogs.cpp:2081 | `if (materializr::lengthField(materializr::trFormat("Width (%s)", materializr::unitSuffix()).c_str(), &widthMM,` |
| CONVERTED | src/app/Application_Dialogs.cpp:2305 | `(void)materializr::parseLength(m_refImgDistBuf, distMM);` |
| count | src/app/Application_Dialogs.cpp:2442 | `if (materializr::stepperRow("patchDetailStep", &f, /*allowNegative=*/false,` |
| absolute-mm | src/app/Application_Dialogs.cpp:2462 | `if (materializr::inputNumber("##patchTol3d", &m_patchParams.tol3d,` |
| angle | src/app/Application_Dialogs.cpp:2472 | `if (materializr::inputNumber("##patchTolAng", &deg, 0.1, 1.0, "%.2f")) {` |
| unitless | src/app/Application_Dialogs.cpp:2481 | `if (materializr::inputNumber("##patchTolCurv", &m_patchParams.tolCurv,` |
| CONVERTED | src/app/Application_Dialogs.cpp:2624 | `if (materializr::parseLength(m_sketchMoveBuf[i], mv)) m_sketchMove[i] = mv;` |
| CONVERTED | src/app/Application_Dialogs.cpp:2631 | `if (materializr::lengthSlider("##slider", &m_sketchMove[i], -100.0f, 100.0f)) {` |
| CONVERTED | src/app/Application_Dialogs.cpp:2966 | `if (materializr::parseLength(m_planeOpOffsetBuf, parsed) &&` |
| CONVERTED | src/app/Application_Dialogs.cpp:2977 | `if (materializr::lengthStepperRow("planeOffsetStep", &offsetF, /*allowNegative=*/true,` |
| angle | src/app/Application_Dialogs.cpp:3034 | `materializr::parseFinite(m_planeOpRotBufX, dx);` |
| angle | src/app/Application_Dialogs.cpp:3035 | `materializr::parseFinite(m_planeOpRotBufY, dy);` |
| angle | src/app/Application_Dialogs.cpp:3036 | `materializr::parseFinite(m_planeOpRotBufZ, dz);` |
| angle | src/app/Application_Dialogs.cpp:3254 | `if (materializr::parseFinite(m_revolveAngleBuf, a)) m_revolveAngle = a; }` |
| angle | src/app/Application_Dialogs.cpp:3258 | `if (ImGui::SliderFloat("##revAngSld", &m_revolveAngle,` |
...
| CONVERTED | src/app/Application_Viewport.cpp:2067 | `(void)materializr::parseFinite(m_sketchShapeDimBuf,` |
| CONVERTED | src/app/Application_Viewport.cpp:2069 | `if (touchui::numberField("##bubbleDia", nullptr,` |
| CONVERTED | src/app/Application_Viewport.cpp:2090 | `materializr::lengthField("##bubbleW", &m_sketchShapeDimW);` |
| CONVERTED | src/app/Application_Viewport.cpp:2093 | `materializr::lengthField("##bubbleH", &m_sketchShapeDimH);` |
| CONVERTED | src/app/Application_Viewport.cpp:2113 | `if (!materializr::parseLength(m_sketchShapeDimBuf, mm) \|\| mm <= 0.0) return false;` |
| CONVERTED | src/app/Application_Viewport.cpp:3236 | `(void)materializr::parseFinite(m_dimEditingBuf, dimPadV);` |
| CONVERTED | src/app/Application_Viewport.cpp:3241 | `if (materializr::inputNumber("##dimval", &dimPadV, 0.0, 0.0, padFmt,` |
| angle | src/app/Application_Viewport.cpp:5999 | `(void)materializr::parseFinite(m_sketchGizmoRotateBuf,` |
| angle | src/app/Application_Viewport.cpp:6001 | `bool typedEnter = materializr::inputNumber(` |
| angle | src/app/Application_Viewport.cpp:6014 | `(void)materializr::parseFinite(m_sketchGizmoRotateBuf, deg);` |
| angle | src/app/Application_Viewport.cpp:6031 | `(void)materializr::parseFinite(m_sketchGizmoRotateBuf, deg);` |
| CONVERTED | src/app/Application_Viewport.cpp:7333 | `materializr::lengthField("##dimW", &m_sketchShapeDimW);` |
| CONVERTED | src/app/Application_Viewport.cpp:7336 | `materializr::lengthField("##dimH", &m_sketchShapeDimH);` |
| CONVERTED | src/app/Application_Viewport.cpp:7372 | `const bool entered = materializr::inputNumber(` |
| CONVERTED | src/app/Application_Viewport.cpp:7390 | `return dimIsLen ? materializr::lengthFieldCommit(shown) : shown;` |
| CONVERTED | src/app/Application_Viewport.cpp:7425 | `const bool ok = isLen ? materializr::parseLength(m_sketchDimBuf, v0)` |
| angle | src/app/Application_Viewport.cpp:7426 | `: materializr::parseFinite(m_sketchDimBuf, v0);` |
...
| comment | src/app/Application.cpp:5320 | `// 0.64 mm off-axis at the surface origin) that WON` |
| comment | src/app/Application.cpp:5414 | `// sides at a generous offset (1 mm) so tessellation slack near the` |
| comment | src/app/Application.cpp:5429 | `const double eps = 1.0; // mm` |
| comment | src/app/Application.cpp:6057 | `// lines wrote raw millimetres: placing a dimension under any non-mm unit` |
| comment | src/app/Application.cpp:6093 | `// Hash point positions and circle/arc radii too (quantised to 1e-4 mm)` |
| comment | src/app/Application.cpp:6434 | `// millimetres. 40 mm is a reasonable first view in millimetres and an` |
| comment | src/app/Application.cpp:6494 | `// Measured offsets were 10–50% of a cell. At a 1 mm grid that reads as` |
| comment | src/app/Application.cpp:6495 | `// slightly-fat lines; at 0.1 mm it is most of a cell, i.e. "I can't draw a` |
...
| comment | src/app/Application.h:1612 | `float m_patternDistance = 5.0f; // linear: spacing in mm along chosen axis` |
| comment | src/app/Application.h:2067 | `// (0.1 / 1 / 10 mm) and gets a solid-blue border when snap is on. Click` |
...
| comment | src/app/Application_Dialogs.cpp:2303 | `// Typed in the display unit (or with its own suffix); the model wants mm.` |
| comment | src/app/Application_Dialogs.cpp:2313 | `// mm-per-pixel from the picked pair → full-frame physical width.` |
| allowed-by-hand | src/app/Application_Dialogs.cpp:2460 | `ImGui::Text("%s", materializr::tr("Gap tolerance (mm)"));` |
| allowed-by-hand | src/app/Application_Dialogs.cpp:2520 | `ImGui::TextDisabled(materializr::tr("Gap %.4f mm  -  tangency %.2f deg"),` |
| comment | src/app/Application_Dialogs.cpp:2787 | `// and print a literal, so choosing "1" under centimetres stored 10 mm and` |
| comment | src/app/Application_Dialogs.cpp:2849 | `// literal mm under a header that already said "(cm)" or "(in)", so the` |
| comment | src/app/Application_Dialogs.cpp:2850 | `// label named one unit while the button set another - 1 meant 1 mm while` |
| comment | src/app/Application_Dialogs.cpp:2851 | `// the popup claimed centimetres. m_sketchGridStep stays mm; only the` |
| comment | src/app/Application_Dialogs.cpp:2974 | `// in, and dragging a -100..100 slider cannot land on 12.5 mm. Same row the` |
...
| comment | src/app/Application_Viewport.cpp:1090 | `// coarsen them pinned the trim/pick radius at its 10 mm cap the` |
| comment | src/app/Application_Viewport.cpp:1651 | `// Push/pull STARTS at 0 mm (no change), and drawDim draws` |
| comment | src/app/Application_Viewport.cpp:1770 | `// the analogue of the mm readout for moves. Uses the cached pivot` |
| comment | src/app/Application_Viewport.cpp:1836 | `// format hid everything under 0.1 mm. (No <cstring> needed -` |
| comment | src/app/Application_Viewport.cpp:1838 | `auto fmtLen = [](char* out, size_t n, float v) {   // v is mm; prints display unit` |
| identifier/other | src/app/Application_Viewport.cpp:2112 | `double mm = 0.0;   // "2in" honoured; bare = display unit` |
| identifier/other | src/app/Application_Viewport.cpp:2113 | `if (!materializr::parseLength(m_sketchShapeDimBuf, mm) \|\| mm <= 0.0) return false;` |
| identifier/other | src/app/Application_Viewport.cpp:2114 | `v = static_cast<float>(mm);` |
| comment | src/app/Application_Viewport.cpp:2615 | `// Cursor in sketch mm - the space labelOffX/Y lives in, so a` |
| comment | src/app/Application_Viewport.cpp:3263 | `// Convert to mm FIRST, then halve a circle's` |
| comment | src/app/Application_Viewport.cpp:4782 | `// 0.5 mm + 0.5 % of view distance instead; an actually-` |
| comment | src/app/Application_Viewport.cpp:4783 | `// occluding face is normally many mm in front.` |
| comment | src/app/Application_Viewport.cpp:5241 | `// ~ 0.85 mm at 240 dpi is unhittable). Still clamped` |
| comment | src/app/Application_Viewport.cpp:5683 | `// 0.5 mm line nudge → 1 mm mirror shift on a 1 mm grid).` |
| comment | src/app/Application_Viewport.cpp:6211 | `// user lands within 0.4 mm of the previous click -` |
| comment | src/app/Application_Viewport.cpp:7387 | `// The pad edits in the display unit; the floor is mm.` |
| comment | src/app/Application_Viewport.cpp:7421 | `// Converting those display->mm made a typed 180 deg arrive` |
