...
def main():
    ...
    lit  = [(classify_literal(f, code), f, ln, code.strip())
    ...
    with open(os.path.join(ROOT, "docs/units-audit-allow.txt"), "w") as a:
        for d, f, ln, _ in lit:
            if d in ("comment", "export/import-format", "diagnostic", "platform-string", "identifier/other"):
                a.write("%s:%d:\n" % (f, ln))
